...
- Works の作例は薄いブラウザ枠の中に置き、縦長のフルページ静止画を CSS `transform` でなぞって見せます。ホバーできる環境ではポインタが乗っているあいだだけ、タッチ端末では画面内に入っているあいだだけ動かします。画面録画の動画は使いません。
- レスポンシブの作例は、端末モックではなく PC / iPad / スマホの比率のブラウザ枠を3つ並べます。`lib/works.ts` に `tablet` と `mobile` の画像があれば3枠、なければ1枠です。
...
lib/                  実績データの型定義とデータ本体
public/works/         実績のスクリーンショット(縦長のフルページ静止画)
```
